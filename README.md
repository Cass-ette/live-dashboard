...
**工作原理**：`agent.py` 通过 AppleScript (`osascript`) 获取前台应用名和窗口标题，每 5 秒向后端上报一次。电量信息通过 `psutil.sensors_battery()` 获取。
...
- **URL 验证**：Agent 验证 `server_url` 必须以 `http://` 或 `https://` 开头（支持内网 HTTP 部署）
